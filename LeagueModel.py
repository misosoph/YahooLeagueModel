...
for start in xrange(1,2000,25):
    response = json.loads(yfs._get('leagues;league_keys='+league_string+'/players;start='+str(start)+"/stats;type=season;season=2015").content)
...
playerStatsFrame['name'] = playerFrame.apply(lambda x: x['name']['full'], axis=1)
...
#%%
statMeans = playerStatsFrame[playerStatsFrame.columns.tolist()[2:]].apply(lambda x:(x-x.mean())/x.std())
